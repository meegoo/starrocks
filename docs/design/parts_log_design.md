...
**内存占用估算**：
- 每条日志约 200-300 字节
- 假设高负载场景：110条/秒 × 600秒 = 约6.6万条/10分钟
- 推荐缓冲区大小：5万条（约10分钟数据）
- 内存占用：5万条 × 300字节 ≈ 15MB per CN
...
private:
    ...
    // 配置参数
    size_t _max_buffer_size = 50000;        // 最大缓存条目数（约10分钟数据）
    int64_t _retention_seconds = 600;       // 保留10分钟
...
void TabletWriteLogManager::_evict_old_logs() {
    ...
    int64_t cutoff_time = now - _retention_seconds;  // 默认10分钟
...
# 内存缓冲区最大条目数（建议值：50000）
tablet_write_log_max_buffer_size = 50000

# 内存日志保留时间（秒），默认10分钟（600秒）
# 超过此时间的日志会被自动从内存中清理
tablet_write_log_retention_seconds = 600
```

**配置说明**：
- **enable_tablet_write_log**：默认关闭，避免影响未使用该功能的用户
- **max_buffer_size**：5万条约占用15MB内存，保留10分钟数据（FE Syncer每60秒同步，10分钟足够覆盖同步延迟）
- **retention_seconds**：10分钟足够覆盖同步延迟和实时查询需求，历史数据已在持久化表中
...
- **BE/CN内存增加**：约15MB per CN（保留10分钟数据）
...
### 12.2 限制
1. **内存表数据有限**：BE内存仅保留10分钟，CN重启后丢失
   - **缓解**：持久化表保留7天历史数据（可配置），10分钟足够覆盖FE同步延迟
...
- ✅ **BE轻量化**：BE仅负责内存管理（保留10分钟），无I/O开销
- ✅ 性能影响极小（< 0.1%，仅内存写入）
- ✅ 内存占用极小（约15MB per CN，保留10分钟数据）
