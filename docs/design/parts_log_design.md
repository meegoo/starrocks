...
**内存占用估算**：
- 每条日志约 200-300 字节
- 假设高负载场景：110条/秒 × 3600秒 = 约40万条/小时
- 40万条日志约 80-120 MB
- 推荐缓冲区大小：50万条（保留约1小时的数据，带冗余）
...
private:
    ...
    // 配置参数
    size_t _max_buffer_size = 500000;       // 最大缓存条目数（约1小时数据）
    int64_t _retention_seconds = 3600;      // 保留1小时
...
void TabletWriteLogManager::_evict_old_logs() {
    ...
    int64_t cutoff_time = now - _retention_seconds;  // 默认1小时
...
# 内存缓冲区最大条目数（建议值：500000）
tablet_write_log_max_buffer_size = 500000

# 内存日志保留时间（秒），默认1小时（3600秒）
# 超过此时间的日志会被自动从内存中清理
tablet_write_log_retention_seconds = 3600
```

**配置说明**：
- **enable_tablet_write_log**：默认关闭，避免影响未使用该功能的用户
- **max_buffer_size**：50万条约占用100-150MB内存，保留1小时的数据足够（FE Syncer每60秒同步一次）
- **retention_seconds**：1小时足够覆盖同步延迟和短期查询需求，历史数据已在持久化表中
...
**缓冲区内存开销**：
- 高负载场景：110条/秒 × 3600秒 = 约40万条/小时
- 推荐配置：50万条缓冲区（带冗余）
- 内存占用：50万条 × 300字节 ≈ 150MB per CN
- **结论**：内存开销适中，对CN影响可忽略
...
- **BE/CN内存增加**：约150MB per CN（保留1小时数据）
...
### 12.2 限制
1. **内存表数据有限**：BE内存仅保留1小时，CN重启后丢失
   - **缓解**：持久化表保留7天历史数据（可配置），1小时足够覆盖实时查询需求
...
- ✅ **BE轻量化**：BE仅负责内存管理（保留1小时），无I/O开销
- ✅ 性能影响极小（< 0.1%，仅内存写入）
- ✅ 内存占用可控（约150MB per CN，保留1小时数据）
