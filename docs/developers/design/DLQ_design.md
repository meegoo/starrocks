...
Source Data ──→ [Parse/Transform] ──→ [Quality Check] ──→ db.table
             ...
             _statistics_._dead_letter_q            SELECT raw_record->>'col'
             WHERE target_table = '...'             FROM _statistics_._dead_letter_q
...
| 生成 | BE 的 DLQ Writer 在 `append()` 时生成 UUID（`generate_uuid_string()`） |
| 为什么需要 | `DLQSyncDaemon` 通过 Stream Load 写入系统表，网络超时等场景下可能重试。PK 表保证同一 `id` 的记录只存一份，实现 at-least-once + 幂等 = exactly-once 语义 |
...
| 字段 | 来源 | 不纳入原因 |
|------|------|-----------|
| `status` / `retry_count` / `resolved_at` | 初版设计 | DLQ 表定位为数据记录，不是工作流引擎。重处理由用户通过 SQL 自行编排（查询 DLQ → 修正 → INSERT INTO 目标表 → DELETE FROM DLQ）。虽然 PK 表支持 UPDATE，但引入状态管理会增加使用复杂度。 |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `dlq_sync_interval_sec` | `30` | `DLQSyncDaemon` 后台同步间隔 |
| `dlq_sync_max_batch_rows` | `10000` | 单次同步最大行数 |
| `dlq_local_retention_hours` | `24` | 本地 DLQ 文件最长保留时间 |
...
DLQ 记录先写入 BE 本地文件（与现有 rejected record 相同的路径），由后台守护线程定期批量同步到 `_statistics_._dead_letter_q` 表。
...
│  └──────────────────────────────────────────────────┘                │
│                                                                       │
│  主导入完成后，DLQ 文件路径上报给 FE (通过 TReportExecStatusParams)    │
└──────────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────┐
│                  后台同步（异步，批量事务）                             │
│                                                                       │
│  BE DLQSyncDaemon (后台线程，类似 load_error_log 清理线程):           │
│                                                                       │
│    每 N 秒 / 积累到 M 条记录:                                         │
│      1. 扫描本地 DLQ 文件目录                                         │
│      2. 读取多个 DLQ 文件 → 合并构造 Chunk                            │
...
│    → 100 个本地 DLQ 文件                                               │
│    → 1 个批量 Stream Load 事务写入 100 条 DLQ 记录                     │
└──────────────────────────────────────────────────────────────────────┘
```

##### DLQ 本地文件格式

DLQ 本地文件使用 JSON Lines 格式（每行一个 JSON 对象），文件路径复用现有的 `LoadPathMgr` 管理：
...
`id` 字段由 BE 的 DLQ Writer 在 append 时生成（`generate_uuid_string()`），写入本地文件。后台同步时原样写入 PK 表，保证重试幂等。
...
```cpp
class DLQSyncDaemon : public Thread {
    void run() {
        while (!_stopped) {
    ...
    void scan_and_sync() {
        ...
        auto chunk = read_and_merge(files, max_batch_rows);
        ...
        auto status = internal_stream_load(chunk, {
            ...
            .target_table = "_statistics_._dead_letter_q",
...
1. **merge commit**：`DLQSyncDaemon` 的 Stream Load 开启 `enable_merge_commit`，FE 将多个 BE 同一时间窗口内的 DLQ Stream Load 合并到同一个事务。这样 N 个 BE 同时同步，最终只产生 1 个事务（而非 N 个）。

2. **PK 去重保证幂等**：DLQ 表是 Primary Key 表，每条 DLQ 记录有 BE 生成的 UUID（`id` 列）。如果 Stream Load 因网络超时等原因需要重试，相同 `id` 的记录会被 PK 表自动去重。这实现了 **at-least-once 投递 + PK 去重 = exactly-once** 语义。

```
BE-1 DLQSyncDaemon ─── Stream Load (merge commit) ──┐
BE-2 DLQSyncDaemon ─── Stream Load (merge commit) ──┤──→ FE merge commit ──→ 1 个事务
BE-3 DLQSyncDaemon ─── Stream Load (merge commit) ──┘
```
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `dlq_sync_interval_sec` | `30` | 后台同步间隔（秒） |
| `dlq_sync_max_batch_rows` | `10000` | 单次同步最大行数 |
| `dlq_local_retention_hours` | `24` | 本地文件最长保留时间（同步失败兜底） |
...
```
t=0    导入开始
t=5    导入完成，DLQ 文件写入 BE 本地磁盘
       用户查 _statistics_._dead_letter_q → 暂无数据
t=35   DLQSyncDaemon 触发同步，读取本地文件，通过 merge commit 批量写入
       用户查 _statistics_._dead_letter_q → 数据可见
```

默认 30 秒同步间隔，用户在导入完成后最多等待 30 秒即可查询 DLQ 数据。对于 `inspect + replay` 的使用模式完全可接受。
...
3. **本地文件写入**：DLQ 记录序列化为 JSON Lines 写入 BE 本地 DLQ 文件。写入路径与现有 `append_rejected_record_to_file()` 相同——本地 append，零事务开销，不影响导入延迟。
4. **后台异步同步**：BE 上的 `DLQSyncDaemon` 守护线程定期扫描本地 DLQ 文件，聚合多个文件/多个导入任务的坏行，通过 1 次 Stream Load 批量写入 `_statistics_._dead_letter_q`。
5. **Best-effort**：本地文件写入失败时静默跳过。后台同步失败时保留文件下次重试，`dlq_local_retention_hours` 超时后清理。
6. **生命周期**：DLQ Writer 随 fragment 创建/销毁，fragment 结束时确保文件 flush。`DLQSyncDaemon` 是 BE 级别的常驻守护线程。

#### 5.1.6 关键设计约束

**约束一：导入路径零事务开销**

DLQ 在导入路径上只做本地文件 append，不开事务、不做 BRPC、不与 FE 交互。所有事务开销转移到后台 `DLQSyncDaemon`，与主导入完全解耦。

**约束二：数据可见性有秒级延迟**

DLQ 数据在导入完成后不立即可见，需等待 `DLQSyncDaemon` 的下一次同步周期（默认 30 秒）。对于"导入完成后手动检查 + replay"的使用模式完全可接受。

**约束三：递归防护**

`DLQSyncDaemon` 通过内部 Stream Load 写入 `_statistics_._dead_letter_q` 时，对该写入禁用 DLQ 功能（RuntimeState 检查目标表名）。
...
struct TQueryOptions {
    ...
    optional i64 dlq_max_records;                    // 单次导入最大 DLQ 记录数
}
...
struct TReportExecStatusParams {
    ...
    optional i64 dlq_records_written;                // BE 上报 DLQ 写入文件的记录数
    optional string dlq_file_path;                   // BE 本地 DLQ 文件路径（供 FE 记录）
}
```

DLQ 的 Thrift 接口非常轻量：FE 只需在 `TQueryOptions` 中设置 DLQ 开关和行数限制，不需要下发 DLQ 表的 tablet 信息（后台同步由 BE 自行发起 Stream Load）。
...
DLQ 在导入路径上只做本地文件 append（与现有 rejected record 相同），不增加额外延迟。后台 `DLQSyncDaemon` 通过 merge commit 批量写入系统表，事务开销极低。
...
结论：**不截断，支持到 JSON 类型的最大值**。`raw_record` 是 JSON 类型，StarRocks JSON 类型支持最大 16MB。对于行级数据记录，不应截断——截断会导致数据不完整无法 replay，违背 DLQ 的核心设计目标。如果个别记录极大导致存储压力，通过 `dead_letter_queue_max_records` 限制总记录数来控制。
...
结论：**使用 EXISTS 子查询或内联谓词函数，而非展开 IN 列表**（详见 8.2 节）。
...
结论：**使用 DLQ 专用的 merge commit label prefix，merge commit 窗口 10 秒**。`DLQSyncDaemon` 的 Stream Load 使用固定的 label prefix（如 `_dlq_sync_`），FE 的 merge commit 机制按 label prefix + 目标表分组，自然隔离 DLQ 同步与用户的 merge commit 负载。merge commit 窗口设置为 10 秒，`dlq_sync_interval_sec` 默认 30 秒，确保同一批次的多 BE 同步落入同一个 merge commit 窗口。
