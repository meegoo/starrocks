...
之前考虑的方案（内部 Stream Load / FE 注入第二个 Sink / 独立 DLQ txn）都在主导入路径上同步写入 DLQ 表，核心问题是**事务开销**：

- 每个导入任务至少 +1 个 DLQ 事务（begin_txn + commit_txn）
- Stream Load 小文件场景：可能只有 1 条坏行，也要开一个 DLQ 事务
- Routine Load：每个 task 一个 DLQ 事务，task 数量可能很大
- 导入路径引入额外的 NodeChannel/BRPC 开销，增加延迟

##### 推荐方案：BE 本地文件缓存 + 后台批量同步
...
| 维度 | 同步写入（轻量 Sink / 独立 txn） | 本地缓存 + 后台同步（推荐） |
|------|-------------------------------|--------------------------|
| 导入路径事务开销 | 每导入 +1 txn | **0**（导入路径只写本地文件） |
| 导入路径延迟影响 | 有（NodeChannel/BRPC） | **无**（本地文件 append，与现有 rejected record 相同） |
| DLQ 数据可见延迟 | 实时（主导入 commit 后即可查询） | 秒级~分钟级（后台同步周期） |
| 数据安全性 | DLQ txn 失败则丢失 | BE 本地文件兜底，同步失败可重试 |
| 实现复杂度 | 高（FE planner + BE Sink + txn 管理） | **低**（BE 文件写入已有；后台同步复用 Stream Load） |
| 小导入场景 | 1 条坏行 = 1 个 txn = 不划算 | 多个小导入的坏行聚合为 1 个批量 txn |
| FE 改动 | 大（Planner 注入 DLQ Sink，TDLQSinkInfo） | **小**（TQueryOptions 增加 DLQ 开关即可） |
...
class DLQSyncDaemon : public Thread {
    void run() {
        while (!_stopped) {
            sleep(sync_interval_sec);  // 默认 10s
            scan_and_sync();
        }
    }
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `dlq_sync_interval_sec` | `10` | 后台同步间隔（秒） |
...
t=15   DLQSyncDaemon 触发同步，读取本地文件，批量写入系统表
       用户查 _statistics_._dead_letter_q → 数据可见
```

默认 10 秒同步间隔，用户在导入完成后最多等待 10 秒即可查询 DLQ 数据。对于 `inspect + replay` 的使用模式（导入完成后手动查询分析），秒级延迟完全可接受。
...
DLQ 数据在导入完成后不立即可见，需等待 `DLQSyncDaemon` 的下一次同步周期（默认 10 秒）。对于"导入完成后手动检查 + replay"的使用模式完全可接受。
