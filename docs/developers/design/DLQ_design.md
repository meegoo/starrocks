...
│   │  sys._dead_letter_q       │   │  SELECT parse_json(raw_record│    │
│   │                           │   │  FROM sys._dead_letter_q     │    │
...
| **原始数据** | `raw_line` char(1024), `raw_field_value` char(1024) | 无 | `raw_message` (String) | record (无长度限制) | **核心字段**，VARCHAR(65535)；列式格式存储重建行 |
...
```sql
CREATE TABLE sys._dead_letter_q (
    -- ① 时间（分区键）
    ...
    -- ⑤ 原始数据 / 重建行
    raw_record          VARCHAR(65535)  COMMENT '被拒绝的记录内容 (原始文本行 或 rebuild_csv_row 重建行)',
...
| 列 | 类型 | 说明 |
|----|------|------|
| `raw_record` | `VARCHAR(65535)` | 被拒绝的记录内容。**不同格式和拒绝阶段的语义不同**（见下文 5.1 节 DLQ Writer 设计）。 |
| `record_format` | `VARCHAR(16)` | 记录原始数据格式，重处理时需要知道按什么格式解析。Redshift/ClickHouse 均为隐含（仅支持特定格式），StarRocks 支持多种格式需显式记录。 |

**`raw_record` 的内容因拒绝阶段和数据格式而异**：

| 拒绝阶段 | CSV | JSON | Parquet/ORC |
|---------|-----|------|-------------|
| Scanner 阶段（解析/类型转换） | 原始文本行 `record.to_string()` | 原始 JSON 对象 `raw_json()` | 无原始行概念 → 列级错误信息（`file=, column=, raw_data=`） |
| Sink 阶段（约束检查/分区/长度） | `chunk->rebuild_csv_row()` 重建的 CSV 行 | 同左（此时原始文本已不可用） | 同左 |

这个设计反映了一个关键现实：**对于列式格式（Parquet/ORC），不存在"原始行"概念**。Parquet/ORC 以列式 batch 方式读取，单行原始 bytes 在引擎中不可获取。此时 `raw_record` 存储的是 `rebuild_csv_row()` 产生的重建行（即 StarRocks 内部列值的 CSV 格式拼接），或列级错误上下文。这与 Redshift 的设计一致——Redshift 的 `raw_line` 同样是最大努力的行级近似。

**为什么不用 1MB 上限**：VARCHAR(65535) 是 StarRocks VARCHAR 默认最大长度（64KB），对于行级数据记录已足够。真正超大的原始记录（如嵌套 JSON）会被截断，截断事实记录在 `error_message` 中。Redshift 更激进——仅 char(1024)。
...
| `raw_field_value` | Redshift | Redshift 额外记录出错字段的原始值。`raw_record` 已包含行级内容，`error_column` 已标识出错列。 |
...
DLQ 表位于已有的 `sys` 系统数据库下，全限定名为 `sys._dead_letter_q`。用户通过标准 SQL 查询和操作，Row Access Policy 自动过滤可见行。

#### 4.6.1 查询 DLQ

```sql
-- 查看某个目标表的 DLQ 数据（普通用户自动按权限过滤）
SELECT * FROM sys._dead_letter_q
...
ALTER TABLE sys._dead_letter_q SET ("partition_live_number" = "14");

-- 手动清理整个 DLQ（需 admin 权限）
TRUNCATE TABLE sys._dead_letter_q;

-- 清理某个目标表的历史 DLQ 数据
DELETE FROM sys._dead_letter_q
WHERE target_database = 'db1'
  AND target_table = 'orders'
...
| 数据内容 | `record \t error_msg \t source` | 结构化 14 列 |
| 实现入口 | `append_rejected_record_to_file()` | `DLQWriter::append()` (替代同一调用点) |
...
class DLQWriter {
public:
    // 在现有 append_rejected_record_to_file() 的每个调用点替换调用
    void append(
        const std::string& record,         // 原始行文本 或 rebuild_csv_row 重建行
...
private:
    DLQRecordBuffer _buffer;               // 内存缓冲
    // target_database, target_table, load_label 等上下文
    // 从 RuntimeState 获取（一次性设置）
};
```
...
// 改造后:
if (state->enable_dlq()) {
    state->dlq_writer()->append(
        chunk->rebuild_csv_row(j, ","),     // record
...
| 拒绝点 | `raw_record` | `record_format` | `error_code` | `error_column` | `source_info` |
|--------|-------------|----------------|-------------|---------------|--------------|
| CSV Scanner 解析失败 | 原始 CSV 文本行 | `csv` | `PARSE_ERROR` / `COLUMN_MISMATCH` | 有（如果可确定） | `{"file":"...","line":N}` |
| JSON Scanner 构造失败 | 原始 JSON 对象 | `json` | `PARSE_ERROR` / `TYPE_MISMATCH` | 有 | `{"file":"...","line":N}` / Kafka 元数据 |
| Avro 转换失败 | JSON 序列化的 Avro datum | `avro` | `TYPE_MISMATCH` | 有 | `{"file":"..."}` |
| Parquet Arrow 转换失败 | `"file=X, column=Y, raw_data=Z"` | `parquet` | `TYPE_MISMATCH` | 有 | `{"file":"..."}` |
| ORC 转换失败 | 空字符串 | `orc` | `TYPE_MISMATCH` | 有 | `{"file":"..."}` |
| Sink: NULL 约束 | `rebuild_csv_row()` 重建行 | 继承 Scanner 设置 | `NULL_VIOLATION` | 有 | 空 |
| Sink: VARCHAR 长度超限 | 同上 | 同上 | `VALUE_OUT_OF_RANGE` | 有 | 空 |
| Sink: Decimal 溢出 | 同上 | 同上 | `VALUE_OUT_OF_RANGE` | 有 | 空 |
| Sink: 分区找不到 | 同上 | 同上 | `PARTITION_NOT_FOUND` | 无 | 空 |
| strict mode 过滤 | `rebuild_csv_row()` 从 source chunk | 同上 | `TYPE_MISMATCH` | 有 | 空 |
...
│              │  → 通过内部 Stream Load 写入 sys._dead_letter_q│     │
│              └─────────────────────────────────────────────┘      │
└───────────────────────────────────────────────────────────────────┘
```
...
1. **统一入口**：`DLQWriter::append()` 替代所有 `append_rejected_record_to_file()` 调用点，接口参数对齐 DLQ 表 Schema。
...
3. **异步批量写入**：记录先累积到 `DLQRecordBuffer`，达到行数阈值（`dead_letter_queue_batch_size`）或时间间隔（`dead_letter_queue_flush_interval_ms`）后，构造 Chunk 并通过内部 Stream Load 写入 `sys._dead_letter_q`。
...
**DLQ 表创建时机**：`sys._dead_letter_q` 在 FE 启动时由 `DLQManager` 初始化（类似 `information_schema` 的系统表初始化流程），而非懒创建。这保证了全局只有一张表，且任何导入任务启用 DLQ 时无需等待建表。
...
FE 启动
  │
  ▼
DLQManager 初始化 sys._dead_letter_q
...
**实现位置**：`QueryAnalyzer` 识别到 `sys._dead_letter_q` 时，调用 `Authorizer` 收集当前用户有 SELECT 权限的 `(database, table)` 集合，作为谓词注入。当集合过大时，优化为与权限元数据表的 semi-join。
...
|------|-------------------|-------------|
| 存储位置 | BE 本地文件系统 | StarRocks 全局 OLAP 表 `sys._dead_letter_q` |
...
| 错误分类 | 无结构化分类 | 标准错误码体系 |
| 重处理能力 | 无 | `INSERT INTO target SELECT ... FROM sys._dead_letter_q` |
...
1. **`sys` 数据库扩展**：当前 `sys` 库仅包含虚拟系统表（`SysDb`），DLQ 表是一张真实的 OLAP 表，是否需要扩展 `SysDb` 以支持混合虚拟表和物理表？还是放在 `_statistics_` 库（已有 OLAP 表的先例）中？
2. **DLQ 写入失败的处理**：best-effort 是否足够？是否需要 fallback 到本地文件？
3. **大量坏数据场景**：Broker Load 导入 TB 级文件，坏行比例高时，DLQ 写入可能成为瓶颈。是否需要采样机制或自动降级。
4. **`raw_record` 截断策略**：超过 65535 字节的记录截断并在 `error_message` 中标注即可，还是需要其他策略？
5. **Parquet/ORC 的 `raw_record` 内容**：当前设计中列式格式的 `raw_record` 是 `rebuild_csv_row()` 重建行或列级错误上下文，是否需要更结构化的表示（如 JSON 格式的 column→value 映射）？
...
- [ ] `sys._dead_letter_q` 表初始化（FE DLQManager）
