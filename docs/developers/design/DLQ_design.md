...
每行内容：
```json
{"created_at":"2026-03-27 10:30:00","target_database":"db1","target_table":"orders","load_label":"load_001","load_type":"INSERT","txn_id":12345,"user_name":"root","error_code":"TYPE_MISMATCH","error_message":"Cannot cast 'abc' to INT","error_column":"amount","raw_record":{"order_id":"10001","customer_name":"Alice","amount":"abc"},"source_info":{"format":"parquet","file":"gs://bucket/data.parquet"},"backend_id":10001}
```
...
class DLQSyncDaemon : public Thread {
    // 定期扫描本地 DLQ 文件目录
    // 聚合多个文件 → 构造 Chunk → 内部 Stream Load
    void run() {
        while (!_stopped) {
...
| 场景 | 同步方案 txn 数 | 本地缓存方案 txn 数 |
|------|---------------|-------------------|
| 100 个 Stream Load，各 1 条坏行 | 100 | **1**（聚合为 1 次批量同步） |
| Routine Load 100 个 task，各 5 条坏行 | 100 | **1~5**（按同步周期聚合） |
| 1 个大 Broker Load，10000 条坏行 | 1 | **1**（单次同步） |
...
8. **多 BE 同步冲突**：多个 BE 的 `DLQSyncDaemon` 同时通过 Stream Load 写入同一张 DLQ 表，是否有并发问题？（Duplicate Key 表无冲突，但需控制 Stream Load 并发数。）
