...
> **建议**：采用 **`_rejected_records`**。虽然 "Queue" 源自消息队列，但 DLQ 作为概念已泛化到整个数据处理领域（包括 ETL 和批处理），ClickHouse 在数据库领域已有先例，且这个名字最具辨识度。同时在文档和 SQL 示例中统一使用 "DLQ" 作为缩写。如果团队倾向更精确的命名，**`_load_errors`** 是最佳替代。
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `rejected_record_ttl_days` | `7` | DLQ 表默认分区保留天数（`partition_live_number`） |
...
DLQ 表位于 `_statistics_` 内部数据库下，全限定名为 `_statistics_._rejected_records`。用户通过标准 SQL 查询和操作，Row Access Policy 自动过滤可见行。
...
`RejectedRecordWriter` 替代 `append_rejected_record_to_file()`，提供统一的 DLQ 记录收集接口：
...
DLQ 记录先写入 BE 本地文件（与现有 rejected record 相同的路径），由后台守护线程定期批量同步到 `_statistics_._rejected_records` 表。
...
1. **merge commit**：`RejectedRecordSyncDaemon` 的 Stream Load 开启 `enable_merge_commit`，FE 将多个 BE 同一时间窗口内的 DLQ Stream Load 合并到同一个事务。这样 N 个 BE 同时同步，最终只产生 1 个事务（而非 N 个）。
...
4. **后台异步同步**：BE 上的 `RejectedRecordSyncDaemon` 守护线程定期扫描本地 DLQ 文件，聚合多个文件/多个导入任务的坏行，通过 1 次 Stream Load 批量写入 `_statistics_._rejected_records`。
...
DLQ 在导入路径上只做本地文件 append，不开事务、不做 BRPC、不与 FE 交互。所有事务开销转移到后台 `RejectedRecordSyncDaemon`，与主导入完全解耦。

**约束二：数据可见性有秒级延迟**

DLQ 数据在导入完成后不立即可见，需等待 `RejectedRecordSyncDaemon` 的下一次同步周期（默认 30 秒）。对于"导入完成后手动检查 + replay"的使用模式完全可接受。

**约束三：递归防护**

`RejectedRecordSyncDaemon` 通过内部 Stream Load 写入 `_statistics_._rejected_records` 时，对该写入禁用 DLQ 功能（RuntimeState 检查目标表名）。
...
**DLQ 表创建时机**：`_statistics_._rejected_records` 由 `RejectedRecordManager`（FrontendDaemon）在 leader 节点启动后异步初始化，与 `StatisticsMetaManager` 创建统计表的模式一致。`RejectedRecordManager` 检查 `_statistics_` 库中是否已存在 `_rejected_records` 表，不存在则通过 `CreateTableStmt` 创建。
...
DLQ 在导入路径上只做本地文件 append（与现有 rejected record 相同），不增加额外延迟。后台 `RejectedRecordSyncDaemon` 通过 merge commit 批量写入系统表，事务开销极低。
...
| 操作 | 权限要求 |
|------|---------|
| DLQ 数据写入 | 系统内部（BE Rejected Record Writer），用户无直接 INSERT 权限 |
