...
- **凭证明文落盘点**:`Pipe.originSql`(`Pipe.java:94`,`@SerializedName`)、`DESC PIPE` 原样 dump(`ShowExecutor.java:3037`)、审计 redactor `SqlCredentialRedactor`(不含 `sasl.jaas.config`/`ssl.*.password`)。
