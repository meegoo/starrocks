...
作为 StarRocks 的近亲项目，Doris 的对比尤为重要：

| 能力 | 具体实现 |
|------|---------|
| **HTTP REST API** | `/api/compaction/show?tablet_id=X`（tablet compaction 状态）、`/api/compaction/run`（手动触发）、`/api/compaction/run_status`（运行状态）、`/api/compaction_score?top_n=N`（Top-N 高分 tablet） |
| **Prometheus Metrics** | `tablet_base_max_compaction_score`、`tablet_cumulative_max_compaction_score`、`total_rowset_num`、内存追踪 |
| **SHOW PROC** | `SHOW PROC '/cluster_health/tablet_health'`、`SHOW TABLET` 带 `CompactionStatus` URL |
| **配置调优** | 动态并发调整（PR #36672）、cumulative compaction 延迟策略（PR #48759） |
| **2025 路线图** | 规划统一资源管理框架，提供实时资源监控系统表，增强 compaction 可观测性 |

**现状**：Doris 的 compaction 可观测性以 HTTP API + SHOW PROC 为主，缺少持久化历史系统表。2025 路线图才开始规划系统表级别的改进。

### 竞品详细特性补充

#### ClickHouse `system.part_log` 事件类型

`system.part_log` 是持久化的 MergeTree 表，记录以下事件：
- `NewPart` — 新 part 创建（导入）
- `MergePartsStart` / `MergeParts` — merge 开始/完成
- `MutatePartStart` / `MutatePart` — mutation 开始/完成
- `DownloadPart` / `MovePart` / `RemovePart` — part 下载/移动/删除

每条记录包含：`duration_ms`、`rows`、`size_in_bytes`、`read_rows`/`read_bytes`、`merged_from`（源 part 数组）、`peak_memory_usage`、`error`/`exception`、`ProfileEvents`（per-operation 计数器 map）。

#### Snowflake 查询剪枝效果追踪

- `TABLE_PRUNING_HISTORY`（Account Usage 视图）：按小时聚合每张表的扫描分区数 vs 剪枝分区数
- `TABLE_QUERY_PRUNING_HISTORY` / `COLUMN_QUERY_PRUNING_HISTORY`：按 query hash 和过滤列聚合剪枝效果
- `QUERY_HISTORY` 的 `PARTITIONS_SCANNED` 字段：用于对比 clustering 前后的查询性能

#### Databricks Predictive Optimization 系统表

`system.storage.predictive_optimization_operations_history`：
- 追踪自动化 OPTIMIZE/VACUUM/ANALYZE 操作
- 字段包含：操作类型、表标识、状态（成功/失败）、预估 DBU 成本、开始/结束时间
