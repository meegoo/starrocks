...
- 単位：ミリ秒
- 変更可能：Yes
- 説明：自動パーティション作成が諦めるまで、ALTER ジョブが FINISHED_REWRITING 状態で完了するのを待つ最大時間（ミリ秒）。自動パーティション作成が競合するスキーマ変更のキャンセルを試みるが、ジョブがすでに FINISHED_REWRITING（Lake テーブル。データの書き換えは完了しておりジョブは自然に終了する）にある場合に適用される。実際の待機時間は、この値とクライアント要求タイムアウトの 1/2 の小さい方で上限される。0 にすると待機せず、この状況では自動パーティション作成は直ちに失敗する。
- 導入時期：-
