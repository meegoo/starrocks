...
class ParquetBrokerScannerTest : public ::testing::Test {
    std::vector<TBrokerRangeDesc> generate_ranges(const std::vector<std::string>& file_names,
                                                  const std::vector<int>& file_sizes, int32_t num_columns_from_file,
        ...
        for (auto i = 0; i < file_names.size(); ++i) {
            ...
            range.file_size = file_sizes[i];
            range.start_offset = 0;
            range.size = file_sizes[i];
        }
        return ranges;
    }
    ...
    std::unique_ptr<ParquetBrokerScanner> create_parquet_scanner(
        ...
        return std::make_unique<ParquetBrokerScanner>(state, profile, *broker_scan_range, counter);
    }

    void validate(std::unique_ptr<ParquetBrokerScanner>& scanner, const size_t expect_num_rows,
    ...
    void test_column_from_path(const std::vector<std::string>& columns_from_file,
        ...
        if constexpr (is_nullable) {
            ...
            file_sizes = _nullable_file_sizes;
        } else {
        ...
        auto ranges = generate_ranges(file_names, file_sizes, columns_from_file.size(), column_values);
    ...
    ChunkPtr test_json_column(const std::vector<std::string>& columns_from_file,
                              const std::unordered_map<size_t, ::starrocks::TExpr>& dst_slot_exprs,
                              std::string specific_file, int file_size) {
        std::vector<std::string> file_names{specific_file};
        std::vector<std::string> column_names = columns_from_file;
        std::vector<int> file_sizes{file_size};

        auto src_slot_infos = select_columns(columns_from_file, is_nullable);
        auto dst_slot_infos = select_columns(column_names, is_nullable);

        auto ranges = generate_ranges(file_names, file_sizes, columns_from_file.size(), {});
        auto* desc_tbl = generate_desc_tbl(src_slot_infos, dst_slot_infos);
        auto scanner = create_parquet_scanner("UTC", desc_tbl, dst_slot_exprs, ranges);
    ...
    void SetUp() {
        ...
        _nullable_file_sizes = std::vector<int>{404,     /* "/test_data/parquet_data/data_0.parquet",*/
                                                1879,    /* "/test_data/parquet_data/data_1.parquet",*/
                                                208495,  /* "/test_data/parquet_data/data_4095.parquet",*/
                                                209234,  /* "/test_data/parquet_data/data_4096.parquet",*/
                                                209719,  /* "/test_data/parquet_data/data_4097.parquet",*/
                                                415710,  /* "/test_data/parquet_data/data_8191.parquet",*/
                                                414021,  /* "/test_data/parquet_data/data_8192.parquet",*/
                                                415490}; /* "/test_data/parquet_data/data_8193.parquet"};*/
...
private:
    ...
    std::vector<int> _nullable_file_sizes;
};

TEST_F(ParquetBrokerScannerTest, test_nullable_parquet_data) {
    ...
    auto ranges = generate_ranges(_nullable_file_names, _nullable_file_sizes, slot_infos.size(), {});
...
TEST_F(ParquetBrokerScannerTest, test_parquet_data) {
    auto column_names = std::vector<std::string>{
            "col_date",     "col_datetime", "col_char",   "col_varchar",      "col_boolean",       "col_tinyint",
            "col_smallint", "col_int",      "col_bigint", "col_decimal_p6s2", "col_decimal_p14s5", "col_decimal_p27s9",
    };
    auto slot_infos = select_columns(column_names, false);
    auto ranges = generate_ranges(_file_names, _file_sizes, slot_infos.size(), {});
    auto* desc_tbl = generate_desc_tbl(slot_infos, {});
    auto scanner = create_parquet_scanner("UTC", desc_tbl, {}, ranges);
    auto check = [](const ChunkPtr& chunk) {
        auto& columns = chunk->columns();
...
TEST_F(ParquetBrokerScannerTest, test_parquet_data_with_1_column_from_path) {
...
TEST_F(ParquetBrokerScannerTest, test_parquet_data_with_2_column_from_path) {
    std::vector<std::string> columns_from_file = {
...
TEST_F(ParquetBrokerScannerTest, test_parquet_data_with_3_column_from_path) {
...
TEST_F(ParquetBrokerScannerTest, test_to_json) {
    ...
    for (auto& [column_name, expected] : test_cases) {
        ...
        ChunkPtr chunk = test_json_column<true>(column_names, slot_map, parquet_file_name, 10368);
