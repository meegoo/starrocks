...
TEST_F(RowsetTest, SegmentWriteTest) {
    ...
    for (int i = 0; i < seg_infos.size(); ++i) {
        ...
        auto buff = seg_infos[i]->mutable_data();
        raw::stl_string_resize_uninitialized(buff, seg_info->data_size());
        ASSERT_TRUE(rfile->read_fully(buff->data(), seg_info->data_size()).ok());
        auto st = segment_rowset_writer->flush_segment(*seg_info);
        LOG(INFO) << st;
        ASSERT_TRUE(st.ok());
    }
