...
// ============================================================================
// lake_rows_mapper_row_count with subtask_id Tests
// ============================================================================

TEST_F(RowsMapperTest, test_lake_rows_mapper_row_count_with_subtask_id) {
    DataDir* dir = get_stores();
    ASSERT_NE(dir, nullptr);
    ...
    // Create file for subtask
    std::vector<uint64_t> rssid_rowids;
    generate_rssid_rowids(&rssid_rowids, 0, 100, 1);

    auto file_or = lake_rows_mapper_filename(tablet_id, txn_id, subtask_id);
    ASSERT_OK(file_or.status());
    ASSERT_OK(create_rows_mapper_file(file_or.value(), rssid_rowids));

    // Test row count
    auto row_count_or = lake_rows_mapper_row_count(tablet_id, txn_id, subtask_id);
    ASSERT_OK(row_count_or.status());
    ASSERT_EQ(100, row_count_or.value());

    // Cleanup
    ASSERT_OK(fs::delete_file(file_or.value()));
}
...
TEST_F(RowsMapperTest, test_lake_rows_mapper_row_count_file_too_small) {
    ...
    // Create a file that's too small
    ASSERT_OK(create_too_small_file(file_or.value()));

    // Should fail with corruption error
    auto result = lake_rows_mapper_row_count(tablet_id, txn_id, subtask_id);
