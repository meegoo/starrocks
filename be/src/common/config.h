...
// is below this threshold AND no rowset anywhere in the tablet has overlapping
// segments or deletes.
//
...
// reduction. The overlap/deletes check is tablet-wide (not per-level) so rowsets
// in lower-score levels with deletes are never starved by a sparse-but-higher-score
// mid-tier; in that case compaction proceeds on the picked level as a normal round.
