...
Status VerticalCompactionTask::compact_column_group(bool is_key, int column_group_index, size_t column_group_size,
    ...
    // Apply range filter for range-split parallel compaction (only on key columns group)
    if (_context->has_range_split && is_key) {
        if (!_context->range_start_key.empty()) {
        ...
        if (!_context->range_end_key.empty()) {
