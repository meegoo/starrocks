...
Status FlatJsonColumnWriter::_init_flat_writers() {
    ...
    for (size_t i = 0; i < _flat_paths.size(); i++) {
        ...
        // it is where the un-extracted conversation array lives. The write-path
        // sampling gate only fires on PLAIN columns whose first page reaches the
        // minimum sample size, so setting this on DICT-encoded or tiny scalar
        // leaves is a harmless no-op.
