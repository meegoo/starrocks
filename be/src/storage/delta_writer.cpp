...
Status DeltaWriter::_init() {
    ...
    if (_replica_state == Primary) {
        _sync_token = _storage_engine->segment_sync_executor()->create_sync_token(&_opt);
    }
    _set_state(kWriting);
...
Status DeltaWriter::write_segment(const SegmentPB& segment_pb) {
    auto state = get_state();
    if (state != kWriting) {
    ...
    return _rowset_writer->flush_segment(segment_pb);
}
...
Status DeltaWriter::_flush_memtable_async(bool eos) {
    ...
    if (_replica_state == Primary) {
        auto sync_token = _sync_token.get();
        return _flush_token->submit(std::move(_mem_table), eos, [sync_token](std::unique_ptr<SegmentPB> seg, bool eos) {
            auto st = sync_token->submit(std::move(seg), eos);
            if (!st.ok()) {
                LOG(WARNING) << st;
                sync_token->set_status(st);
            }
        });
    }
    return Status::OK();
...
Status DeltaWriter::commit() {
    ...
    if (_replica_state == Primary) {
        if (auto st = _sync_token->wait(); UNLIKELY(!st.ok())) {
            LOG(WARNING) << st;
            _set_state(kAborted);
            return st;
        }
