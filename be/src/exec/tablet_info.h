    ...
    void add_locations(std::vector<TTabletLocation>& locations) {
        for (auto& location : locations) {
            if (_tablets.count(location.tablet_id) == 0) {
                LOG(INFO) << "auto:<<location.tablet_id:" << location.tablet_id << ":size:" << location.node_ids.size();
                _new_tablets.emplace_back(std::move(location));
                _tablets.emplace(location.tablet_id, &_new_tablets.back());
            }
        }
    }
...
private:
    ...
    // tablets create by automatic partition
    std::vector<TTabletLocation> _new_tablets;
};
